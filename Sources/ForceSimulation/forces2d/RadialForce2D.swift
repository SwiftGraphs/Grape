...
final public class RadialForce2D<NodeID>: ForceLike
...
extension RadialForce2D.Strength {
...
extension RadialForce2D.NodeRadius {
    public func calculated(for simulation: Simulation2D<NodeID>) -> [Double] {
        switch self {
        case .constant(let r):
...
extension Simulation2D {
    ...
    public func createRadialForce(
        center: V = .zero,
        radius: RadialForce2D<NodeID>.NodeRadius,
        strength: RadialForce2D<NodeID>.Strength = .constant(0.1)
    ) -> RadialForce2D<NodeID> {
        let f = RadialForce2D<NodeID>(center: center, radius: radius, strength: strength)
        f.simulation = self
