...
//
    ...
    public struct MaxRadiusTreeDelegate2D<NodeID>: QuadtreeDelegate where NodeID: Hashable {
        ...
        @inlinable public mutating func didAddNode(_ nodeId: NodeID, at position: V) {
            let p = radiusProvider(nodeId)
            maxNodeRadius = max(maxNodeRadius, p)
        }

        @inlinable public mutating func didRemoveNode(_ nodeId: NodeID, at position: V) {
        ...
        @inlinable public func copy() -> MaxRadiusTreeDelegate2D {
            return Self(maxNodeRadius: maxNodeRadius, radiusProvider: radiusProvider)
        }

        @inlinable public func spawn() -> MaxRadiusTreeDelegate2D {
