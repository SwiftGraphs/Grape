...
extension Simulation2D.LinkForce.LinkLookup {
...
extension Simulation2D.LinkForce.LinkLength {
    func calculated(
        for links: [EdgeID<NodeID>],
        connectionLookupTable: Simulation2D.LinkForce.LinkLookup<NodeID>
    ) -> [Double] {
        switch self {
...
extension Simulation2D.LinkForce.LinkStiffness {
    func calculated(
        for links: [EdgeID<NodeID>],
        connectionLookupTable lookup: Simulation2D.LinkForce.LinkLookup<NodeID>
    ) -> [Double] {
        switch self {
        case .constant(let value):
            return links.map { _ in value }
