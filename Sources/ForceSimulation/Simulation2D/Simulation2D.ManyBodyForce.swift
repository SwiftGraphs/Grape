...
//
    ...
    public struct MassQuadtreeDelegate2D<NodeID>: QuadtreeDelegate where NodeID: Hashable {
        ...
        @inlinable public mutating func didAddNode(_ node: NodeID, at position: simd_double2) {
        ...
        @inlinable public mutating func didRemoveNode(_ node: NodeID, at position: simd_double2) {
            let p = massProvider(node)
            accumulatedCount -= 1
            accumulatedMass -= p
            accumulatedMassWeightedPositions -= position * p
    ...
    extension Simulation2D.ManyBodyForce.NodeMass {
        @inlinable
